...
class SerpApiProvider(FlightProvider):
    ...
    def search_flights(self, origin: str, destination: str, departure_date: str,
        ...
        try:
            ...
            try:
                ...
                if budget_options:
                    logger.info(f"Found {len(budget_options)} budget airline alternatives")
    ...
    def create_booking(self, offer_id: str, passengers: List[Passenger], 
                    ...
                    emergency_contact: Optional[Any] = None) -> BookingResponse:
    ...
    def _transform_serpapi_response(self, api_response: Dict, origin: str, 
                               destination: str, return_date: Optional[str]) -> Tuple[SimplifiedSearchResponse, FlightSearchResponse]:
        ...
        try:
            ...
            for idx, flight_data in enumerate(all_flights):
                try:
                    flight_offer = self._process_serpapi_flight(flight_data, idx, origin, destination, return_date)
    ...
    def _process_serpapi_flight(self, flight_data: Dict, idx: int, origin: str, 
                            destination: str, return_date: Optional[str]) -> FlightOffer:
        ...
        carbon_emissions_kg = carbon_data.get("this_flight", 0) / 1000 if carbon_data else None
        ...
        provider_data = {
            ...
            "often_delayed": any(seg.get("often_delayed_by_over_30_min") for seg in flights)
        }
    ...
    def _process_serpapi_segment(self, seg_data: Dict, seg_idx: int) -> FlightSegment:
        ...
        departure_time = self._parse_serpapi_datetime(departure_time_str) if departure_time_str else datetime.now()
        arrival_time = self._parse_serpapi_datetime(arrival_time_str) if arrival_time_str else datetime.now()
        ...
        airline_code = flight_number.split()[0] if flight_number else airline[:2].upper()
